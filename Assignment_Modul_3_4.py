...
#def parse_users(users):# TODO: Оформити в функцію
...
def get_upcoming_birthdays(prepared_users, days=7):  # TODO: Оформити в функцію  # 7 Кількість днів для перевірки на наближені дні народження
    today = datetime.today().date()   # Поточна дата
    upcoming_birthdays = []  # Список майбутніх днів народження
    for user in users:  # Ітерація по користувачам
        try:
